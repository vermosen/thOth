...
struct EClientL0Impl: public EPosixClientSocket
{
	...
	bool eConnect( const char *host, unsigned int port, int clientId=0 )
	{
		m_EWrapperL0Impl.EnableCalls();									// just to ensure it is enabled
																		...
																		// If connected, just let it 
		if( isConnected() )												// follow the flow in EClientSocket
			return EPosixClientSocket::eConnect(host, port, clientId);

		if( !EPosixClientSocket::eConnect(host, port, clientId) )
		{
		...
		if( m_EWrapperL0->IsCalledFromThread() )						// Then start the reading thread
		{	
#ifdef WIN32
...
struct EClientL0Interface: public EClientL0, CriticalSection
{
    ...
    { ENTER_CLIENT( reqMktDepth                 , id )
                 EC.reqMktDepth                 ( id, contract, numRows);
    ...
    { ENTER_CLIENT( reqScannerSubscription      , tickerId )
                 EC.reqScannerSubscription      ( tickerId, subscription);
